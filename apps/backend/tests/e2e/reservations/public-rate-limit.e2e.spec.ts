...
beforeAll(async () => {
  for (const key of [
    "PUBLIC_AVAILABILITY_RATE_LIMIT_MAX", "PUBLIC_RESERVATION_CREATE_RATE_LIMIT_MAX",
    "PUBLIC_RESERVATION_ACCESS_RATE_LIMIT_MAX", "PUBLIC_ORDER_CREATE_RATE_LIMIT_MAX",
    "PUBLIC_ORDER_LOOKUP_RATE_LIMIT_MAX", "PUBLIC_ORDER_CANCEL_RATE_LIMIT_MAX",
...
describe("Public rate limiting", () => {
  it.each(["availability", "create", "lookup", "cancel", "order-create", "order-lookup", "order-cancel"])("limits %s by IP across changing identifiers", async kind => {
    ...
    const request = () => {
      ...
      if (kind === "order-create") return app.inject({ remoteAddress, method: "POST", url: `/public/restaurants/${slug}/orders`, headers, payload: {} });
      if (kind === "order-lookup") return app.inject({ remoteAddress, url: `/public/orders/${token}` });
      if (kind === "order-cancel") return app.inject({ remoteAddress, method: "POST", url: `/public/orders/${token}/cancel`, headers });
