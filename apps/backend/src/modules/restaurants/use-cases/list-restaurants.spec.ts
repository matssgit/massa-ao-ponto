...
describe("List Restaurants Use Case", () => {
  ...
  it("lists only active user memberships without duplicates, ordered by name and id", async () => {
    ...
    repository.restaurants = ["b", "a", "c", "d"].map((id) => ({
      ...
      timezone: "UTC", slug: null, publicEnabled: false, deliveryEnabled: false, deliveryFeeCents: 0, whatsappNotificationsEnabled: false, operationalOverride: "DEFAULT", createdAt: now, updatedAt: now,
    }));
