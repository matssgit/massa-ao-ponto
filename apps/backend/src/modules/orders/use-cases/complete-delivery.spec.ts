...
import { CompleteDeliveryUseCase } from "./complete-delivery.use-case.js";
...
describe("CompleteDeliveryUseCase", () => {
  ...
  async function createStartedOrderWithDelivery() {
    ...
    await deliveriesRepository.updateStatus(delivery.id, "OUT_FOR_DELIVERY");
    return { order, delivery };
  }

  it("deve concluir Delivery do próprio tenant e registrar ambos os histories", async () => {
    const { order, delivery } = await createStartedOrderWithDelivery();
