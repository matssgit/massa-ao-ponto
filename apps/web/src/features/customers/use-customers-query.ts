...
export function useCustomersQuery<T>(key: string, load: (signal: AbortSignal) => Promise<T>) {
  ...
  useEffect(() => {
    ...
    void load(controller.signal).then((data) => {
      ...
      setResult({ key, state: { status: "error", message: error instanceof ApiError ? error.message : "Não foi possível carregar os clientes." } });
    });
