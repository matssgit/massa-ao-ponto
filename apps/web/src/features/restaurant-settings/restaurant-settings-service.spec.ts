...
const restaurant = { id: restaurantId, name: "Massa Centro", address: "Rua A, 10", phone: "11999999999", timezone: "America/Sao_Paulo", deliveryEnabled: false, deliveryFeeCents: 0, whatsappNotificationsEnabled: false, operationalOverride: "DEFAULT", pixKey: null, pixRecipientName: null, createdAt: timestamp, updatedAt: timestamp };
...
describe("RestaurantSettingsService", () => {
  ...
  it("trims input and enforces the physical field limits", () => {
    expect(restaurantSettingsInputSchema.parse({ name: " Casa ", address: " Rua ", phone: " 11 ", timezone: " UTC ", deliveryEnabled: false, deliveryFeeCents: 0, whatsappNotificationsEnabled: false, operationalOverride: "DEFAULT" })).toEqual({ name: "Casa", address: "Rua", phone: "11", timezone: "UTC", deliveryEnabled: false, deliveryFeeCents: 0, whatsappNotificationsEnabled: false, operationalOverride: "DEFAULT", pixKey: null, pixRecipientName: null });
    expect(() => restaurantSettingsInputSchema.parse({ name: "A", address: "B", phone: "1", timezone: "x".repeat(101), deliveryEnabled: false, deliveryFeeCents: 0, whatsappNotificationsEnabled: false, operationalOverride: "DEFAULT" })).toThrow();
  });
});
