...
export function useCatalogQuery<T>(key: string, load: (signal: AbortSignal) => Promise<T>) {
  ...
  useEffect(() => {
    ...
    void load(controller.signal).then((data) => {
      ...
      if (!controller.signal.aborted) setResult({ key, state: { status: "error", message: error instanceof ApiError ? error.message : "Não foi possível carregar o cardápio." } });
    });
