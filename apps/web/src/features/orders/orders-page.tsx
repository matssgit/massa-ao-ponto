...
function RestaurantOrders({ restaurantId, owner }: { restaurantId: string; owner: boolean }) {
  ...
  return <section className="orders-page">
    <header className="orders-heading"><div><p className="eyebrow">OPERAÇÃO DA CASA</p><h1 ref={heading} tabIndex={-1}>Pedidos</h1></div>
      {!selected && !creating && <div className="admin-heading-actions"><button className="primary" onClick={() => setCreating(true)}>+ Novo pedido</button><button className="secondary" onClick={reload}>Atualizar listagem</button></div>}
    </header>
    <div hidden={selected !== null || creating}>
    ...
    {creating && <CreateOrderForm client={auth.client} service={service} restaurantId={restaurantId} onCancel={() => setCreating(false)} onUncertain={() => { setCreating(false); reload(); }} onCreated={(order) => { setCreating(false); reload(); setSelected(order.id); }} />}
