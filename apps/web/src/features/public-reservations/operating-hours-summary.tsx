import { restaurantLocalDate, weekdayLabels, type OperatingHours, type OperationalOverride, type SpecialHour } from "../../lib/operating-hours";
import "./operating-hours.css";

export function OperatingHoursSummary({ hours, open, override, specialHours, timezone }: { hours: OperatingHours; open: boolean; override: OperationalOverride; specialHours: SpecialHour[]; timezone: string }) {
  const todaySpecial = specialHours.find(entry => entry.date === restaurantLocalDate(new Date(), timezone));
  return <section className="public-hours" aria-labelledby="public-hours-title">
    ...
    {override === "DEFAULT" && todaySpecial?.label && <p>{todaySpecial.label}</p>}
