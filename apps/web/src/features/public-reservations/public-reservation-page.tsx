...
function ReservationForm({ service, slug, restaurant }: { service: PublicReservationService; slug: string; restaurant: PublicRestaurant }) {
  ...
  async function check(event?: FormEvent) {
    ...
    try {
      ...
      if (!isReservationWithinOperatingHours(restaurant.operatingHours, restaurant.timezone, next.startsAt, next.endsAt, restaurant.operationalOverride, restaurant.specialHours)) throw new Error(restaurant.operationalOverride === "CLOSED" ? "O restaurante está temporariamente fechado." : "O restaurante está fechado nesse período. Escolha um horário dentro do funcionamento informado.");
  ...
  async function submit() {
    ...
    submitting.current = true; setBusy(true); setError(undefined); setUncertain(false);
  ...
  return <><Link className="public-back" to={`/r/${encodeURIComponent(slug)}`}>← {restaurant.name}</Link><div className="public-page-title"><p className="public-eyebrow">Planeje seu encontro</p><h1>Uma mesa para vocês.</h1><p>Escolha o período, encontre sua mesa e reserve.</p></div><div className="public-booking-grid"><div>
    ...
    {error !== undefined && <><ErrorNotice error={error} />{uncertain && <p>Não foi possível confirmar o resultado. A reserva pode ter sido criada. Para evitar duplicidade, não envie novamente; entre em contato com o restaurante.</p>}</>}
    {review && period && selected && <section className="public-card public-review"><h2 ref={summary} tabIndex={-1}><span className="public-step">04</span> Está tudo certo?</h2><p>{customer.name}, sua reserva será em <strong>{restaurant.name}</strong>.</p><p>{period.partySize} pessoas · {selected.type === "room" ? "Sala" : "Mesa"} {selected.number}</p><p>{publicDate(period.startsAt, restaurant.timezone)}<br />até {publicDate(period.endsAt, restaurant.timezone)}</p><p>Fuso: {restaurant.timezone}</p><button className="public-primary" disabled={busy || uncertain} onClick={() => void submit()}>{busy ? "Reservando…" : "Confirmar reserva"}</button></section>}
