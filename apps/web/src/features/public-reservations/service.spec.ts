...
import { availabilitySchema, createdSchema, customerFormSchema, detailsSchema } from "./schemas";
...
describe("Public reservation contracts", () => {
  it("uses exact public endpoints with no cookies or CSRF, no referrer, and required write header", async () => {
    ...
    expect(String(transport.mock.calls[2][1]?.body)).not.toMatch(/customerId|tokenHash/);
    expect(String(transport.mock.calls[4][0])).toBe(`https://api.example.com/public/reservations/${token}/cancel`);
  });
