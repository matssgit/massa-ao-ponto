...
describe("Public pickup order UI", () => {
  ...
  it("validates customer data, reviews PICKUP and sends only ids and quantities once", async () => {
    ...
    expect(body).toEqual({ type: "PICKUP", paymentMethod: "PIX", customer: { name: "Maria Silva", phone: "11987654321" }, items: [{ productId: catalog.categories[0].products[0].id, quantity: 1, addons: [{ addonId: catalog.categories[0].products[0].addons[0].id, quantity: 1 }] }], observation: "Sem cortar" });
