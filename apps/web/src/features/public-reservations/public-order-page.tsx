...
import { publicOrderCustomerFormSchema } from "./schemas";
...
export function PublicOrderPage({ service, slug }: { service: PublicReservationService; slug: string }) {
  ...
  async function submit() {
    ...
    try {
      const result = await service.createOrder(slug, {
        type: "PICKUP",
  ...
  return <PublicFrame slug={slug} page="order"><Link className="public-back" to={`/r/${encodeURIComponent(slug)}`}>← Voltar ao restaurante</Link>
    {query.loading ? <p role="status">Preparando seu pedido…</p> : missing ? <PublicMissing /> : query.error ? <ErrorNotice error={query.error} retry={query.reload} /> : restaurant && catalog && <>
      {step === "menu" && <section aria-labelledby="order-menu-title"><header className="public-page-title"><p className="public-eyebrow">Retirada no local</p><h1 id="order-menu-title">Monte seu pedido</h1><p className="public-intro">Escolha os itens para retirar em {restaurant.name}.</p></header>
        ...
        })}</section>)}</div><aside className="public-card public-order-cart"><h2>Seu carrinho</h2>{lines.length === 0 ? <p>Seu carrinho está vazio.</p> : <ul>{lines.map((line) => <li key={line.product.id}><span>{line.quantity}× {line.product.name}</span><strong>{formatCatalogMoney(line.product.price * line.quantity + line.addons.reduce((sum, entry) => sum + entry.addon.price * entry.quantity, 0))}</strong></li>)}</ul>}<p><span>Total estimado</span> <strong>{formatCatalogMoney(estimate)}</strong></p><small>Estimativa para conferência. O valor final é calculado pelo restaurante no servidor.</small><button className="public-primary" disabled={lines.length === 0} onClick={() => setStep("customer")}>Continuar</button></aside></div>
      </section>}
      ...
      {step === "review" && <section className="public-detail"><h1 ref={stepHeading} tabIndex={-1}>Revise seu pedido</h1><p className="public-status">Retirada no local</p><section className="public-card"><h2>Itens</h2><ul className="public-order-review-items">{lines.map((line) => <li key={line.product.id}><strong>{line.quantity}× {line.product.name}</strong>{line.addons.length > 0 && <ul>{line.addons.map(({ addon, quantity }) => <li key={addon.id}>{quantity}× {addon.name}</li>)}</ul>}</li>)}</ul><p>Total estimado: <strong>{formatCatalogMoney(estimate)}</strong></p><small>O total confirmado na próxima tela é calculado pelo servidor.</small></section><section className="public-card"><h2>Retirada e contato</h2><p>{restaurant.name}<br />{restaurant.address}</p><p>{form.name}<br />{form.phone}{form.email && <><br />{form.email}</>}</p>{form.observation && <p className="public-notes">{form.observation}</p>}</section>{uncertain && <div className="public-error" role="alert" tabIndex={-1}><p>Não foi possível confirmar a resposta. O pedido pode ter sido criado. Para evitar duplicidade, não envie novamente agora; confira sua conexão e entre em contato com o restaurante.</p></div>}{submitError && !uncertain && <ErrorNotice error={submitError} />}<div className="public-confirm-actions"><button className="public-secondary" disabled={busy} onClick={() => setStep("customer")}>Editar dados</button><button className="public-primary" disabled={busy || uncertain} onClick={() => void submit()}>{busy ? "Enviando pedido…" : "Confirmar pedido"}</button></div></section>}
    </>}
  </PublicFrame>;
}
