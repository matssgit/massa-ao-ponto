...
export function ReservationActions({ reservation, busy, onAction }: { reservation: Reservation; busy: boolean; onAction: (action: ReservationAction) => void }) {
  ...
  return <section className="reservation-actions" aria-label="Ações da reserva" aria-busy={busy}><h3>Operar reserva</h3>
    ...
    {currentConfirmation && <div className="reservation-confirmation" role="group" aria-label="Confirmação do cancelamento"><p>Confirma o cancelamento? O servidor verificará novamente status e antecedência antes de alterar a reserva.</p><button className="primary" disabled={busy} onClick={() => onAction(currentConfirmation)}>Confirmar cancelamento</button><button className="secondary" disabled={busy} onClick={() => setConfirmation(null)}>Voltar sem confirmar</button></div>}
